...
async function renderRoute(route) {
  ...
  try {
    ...
    if (route === 'progress') {
      ...
      return renderProgress(progress.progress, submissions.submissions);
    }
...
function renderProgress(progress, submissions) {
  const own = progress.find((entry) => Number(entry.id) === Number(state.user.id));
  els.content.innerHTML = `
    <section class="page-section">
      ${sectionHead('Abgaben', 'Fortschritt der Crew und deine eigenen Einträge')}
      <form id="submission-form" class="panel submit-panel">
        <div class="submit-panel__fields">
          ...
          <label>Notiz<textarea name="note" rows="1" placeholder="Was wurde abgegeben?"></textarea></label>
        </div>
        <button class="button button--primary" type="submit">Abgabe eintragen</button>
      </form>
      ...
      <div class="panel" style="margin-top:24px">
        ...
        <div class="panel__body activity-list">
          ${submissions.length ? submissions.slice(0, 20).map((entry) => `<div class="activity-row"><strong>${escapeHtml(entry.display_name || state.user.display_name)}</strong><span>${formatNumber(entry.amount)}</span><span>${escapeHtml(entry.note || 'Ohne Notiz')}</span><span class="status-badge status-badge--${escapeHtml(entry.status)}">${statusText(entry.status)}</span></div>`).join('') : emptyState('↗', 'Noch keine Abgaben', 'Dein erster Eintrag wird hier angezeigt.')}
        </div>
      </div>
    </section>`;
  ...
  if (own && Number(own.submission_target) === 0 && state.user.is_admin) {
    form.insertAdjacentHTML('afterend', '<p class="form-note" style="margin:-12px 0 18px">Dein Ziel steht aktuell auf 0. Du kannst es im Adminbereich unter Accounts setzen.</p>');
  }
}
...
function adminSubmissions() {
  ...
  return `<div class="panel"><div class="panel__head"><h3>Abgaben prüfen</h3><span class="muted">${rows.filter((item) => item.status === 'pending').length} offen</span></div><div class="panel__body admin-list">
    ${rows.length ? rows.map((item) => `<div class="admin-row"><div class="admin-row__name"><div><strong>${escapeHtml(item.display_name)}</strong><span>${formatDate(item.submitted_at, { dateStyle: 'medium', timeStyle: 'short' })} · ${escapeHtml(item.note || 'Ohne Notiz')}</span></div></div><strong>${formatNumber(item.amount)}</strong><span class="status-badge status-badge--${escapeHtml(item.status)}">${statusText(item.status)}</span><div class="admin-row__actions">${item.status === 'pending' ? `<button class="button button--small" data-review="approved" data-id="${item.id}">Bestätigen</button><button class="button button--small button--danger" data-review="rejected" data-id="${item.id}">Ablehnen</button>` : ''}</div></div>`).join('') : emptyState('↗', 'Keine Abgaben', 'Eingetragene Abgaben erscheinen hier.')}
  </div></div>`;
}
...
async function reviewSubmission(id, status) {
  try {
    await api(`/api/admin/submissions/${id}`, { method: 'PATCH', body: { status } });
    toast(status === 'approved' ? 'Abgabe bestätigt.' : 'Abgabe abgelehnt.');
    await loadAdmin();
  } catch (error) {
    toast(error.message, true);
  }
...
function registerWebMcpTools() {
  ...
  Promise.resolve(context.registerTool({
    ...
    description: 'Trägt eine neue Abgabe für den aktuell angemeldeten Account ein. Sie wartet anschließend auf Admin-Bestätigung.',
    inputSchema: {
      type: 'object',
      properties: {
    ...
    async execute(input) {
      ...
      return { id: result.submission.id, status: result.submission.status, amount: result.submission.amount };
    },
